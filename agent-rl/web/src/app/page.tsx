...
export default function Home() {
  ...
  const handleScenarioSubmit = async (scenario: string, config: { multiplier: number; num_agents: number; max_iterations: number }) => {
    try {
      const response = await fetch('http://localhost:8000/api/submit-scenario', {
        method: 'POST',
  ...
  const handleSeedTaskUpload = async (file: File) => {
    ...
    const response = await fetch('http://localhost:8000/api/upload-seed-tasks', {
      method: 'POST',
      body: formData
    })
  ...
  const handleGenerate = async () => {
    const response = await fetch('http://localhost:8000/api/generate-seed-tasks', {
      method: 'POST'
    })
  ...
  const handleStartTraining = async (config: { learning_rate: number; batch_size: number; num_epochs: number; use_wandb: boolean }) => {
      ...
      <div className="container mx-auto py-8 px-4 relative">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-gray-600 text-base">
            Customize your own agents based on your scenarios and tasks
          </p>
        </div>
